...
void consume_queue_on_connection(MMAL_CONNECTION_T *conn, MMAL_PORT_T *port);
...
void consume_queue_on_connection(MMAL_CONNECTION_T *conn, MMAL_PORT_T *port)
{
	...
	if ((buffer = mmal_queue_get(conn->pool->queue)) != NULL)
	{    
		while(buffer)
		{
			if (mmal_port_send_buffer(port, buffer) != MMAL_SUCCESS)
			{
				mmal_queue_put_back(conn->pool->queue, buffer);
				break;
			}
			else if (port->type == 2) // type 2 -> IN
			{
				buffer = mmal_queue_get(conn->queue);
			} else if (port->type == 3) // type 3 -> OUT
			{
				buffer = mmal_queue_get(conn->pool->queue);
			}
		}
	}    
}
...
int main(void)
{
	...
	vcos_assert((mmal_port_enable(camera->control, camera_control_callback) == MMAL_SUCCESS) && "Failed enabling camera control port");
	...
	vcos_assert((mmal_port_enable(preview->control, preview_control_callback) == MMAL_SUCCESS) && "Failed enabling preview control port");
	...
	while (1)
	{
		static unsigned capturing = 1;
		
		capturing=!capturing;
		mmal_port_parameter_set_boolean(camera->output[1], MMAL_PARAMETER_CAPTURE, capturing);

		vcos_sleep(40);
		consume_queue_on_connection(preview_conn, preview_conn->in);
		consume_queue_on_connection(preview_conn, preview_conn->out);
	}
